...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            resultchip = subprocess.run([qemubinary, "-machine", "help"], capture_output=True, text=True)  # Show machine options to user
        ...
        chipset_models = extract_section(resultchip.stdout.splitlines(), "Supported machines are:")
        ...
        print("Available machine types from your QEMU installation:")
        for chipset in chipset_models:
            print(f"  - {chipset.strip()}")
        print("\n")
        chipmodel = input("Machine type: ").lower() or "q35"
        ...
        # TPM
        if fwtype == "uefi":
            tpm_backend = input("TPM backend (emulator/passthrough/none): ").lower() or "emulator"
            tpm_path = ""
        ...
        # SATA
        sataswitch = input("Enable SATA controller (replaces legacy IDE) (true/false): ").lower() == "true"
        ...
        cfg = {
            "vm": {
                ...
                "cpu": {"model": cpumodel, "sockets": cpusock, "cores": cpucores, "threads": cputhreads},
                "memory": {"size": ramsize},
                "sata": sataswitch,
                "disks": [
